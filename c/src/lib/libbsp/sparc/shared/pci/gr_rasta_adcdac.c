...
int gr_rasta_adcdac_hw_init1(struct gr_rasta_adcdac_priv *priv)
{
	...
	*page0 = 0x80000000;	
	...
	tmp = NULL;
	status = ambapp_for_each(&priv->abus, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_PCIFBRG, gr_rasta_adcdac_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -3;
	}
	...
	/* Find IRQ controller */
	tmp = NULL;
	status = ambapp_for_each(&priv->abus, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_IRQMP, gr_rasta_adcdac_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -4;
	}
	...
	tmp = NULL;
	status = ambapp_for_each(&priv->abus, (OPTIONS_ALL|OPTIONS_AHB_SLVS), VENDOR_GAISLER, GAISLER_PCIFBRG, gr_rasta_adcdac_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -5;
	}
