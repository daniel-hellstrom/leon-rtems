...
int gr_rasta_spw_router_hw_init(struct gr_rasta_spw_router_priv *priv)
{
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_IRQMP, gr_rasta_spw_router_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -4;
	}
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_AHB_SLVS), VENDOR_GAISLER, GAISLER_GRPCI2, gr_rasta_spw_router_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -5;
	}
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_GRPCI2, gr_rasta_spw_router_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -6;
	}
