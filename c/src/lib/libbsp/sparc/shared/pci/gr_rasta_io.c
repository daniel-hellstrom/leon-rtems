...
int gr_rasta_io_init1(struct rtems_drvmgr_dev_info *dev);
int gr_rasta_io_init2(struct rtems_drvmgr_dev_info *dev);
...
struct rtems_drvmgr_drv_ops gr_rasta_io_ops = 
{
 ...
 */
struct rtems_drvmgr_drv_res *gr_rasta_io_resources[] __attribute__((weak)) =
{
	NULL
};
...
void gr_rasta_io_isr (int irqno, struct gr_rasta_io_priv *priv)
{
...
int gr_rasta_io_dev_find(struct ambapp_dev *dev, int index, int maxdepth, void *arg)
{
...
int gr_rasta_io_hw_init(struct gr_rasta_io_priv *priv)
{
	...
	*page0 = 0x80000000;	
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_PCIFBRG, 10, gr_rasta_io_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -3;
	}
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_APB_SLVS), VENDOR_GAISLER, GAISLER_IRQMP, 10, gr_rasta_io_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -4;
	}
	...
	tmp = NULL;
	status = ambapp_for_each(priv->abus.root, (OPTIONS_ALL|OPTIONS_AHB_SLVS), VENDOR_GAISLER, GAISLER_PCIFBRG, 10, gr_rasta_io_dev_find, &tmp);
	if ( (status != 1) || !tmp ) {
		return -5;
	}
	ahb = (struct ambapp_ahb_info *)tmp->devinfo;
	priv->bus_maps[2].map_size = ahb->mask[0]; /* AMBA->PCI Window on GR-RASTA-IO board */
